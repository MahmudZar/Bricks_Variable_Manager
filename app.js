document.addEventListener("DOMContentLoaded", () => {
  ...
  function cacheDomElements() {
    dom = {
      ...
      clearCategorySelectionBtn: document.getElementById(
        "clear-category-selection-btn"
      ),
      bulkDeleteCategoryBtn: document.getElementById(
        "bulk-delete-category-btn"
      ),
      addVariableBtn: document.getElementById("add-variable-btn"),
      selectAllVarsCheckbox: document.getElementById(
        "select-all-vars-checkbox"
      ),
  ...
  function ensureUncategorizedExists() {
    ...
    const uncategorized = state.data.categories.find(
  ...
  function getVisibleVariables() {
    ...
    if (state.searchQuery) {
      ...
      return state.data.variables.filter(
        (v) =>
    ...
    return state.data.variables.filter(
      (v) => v.category === state.activeCategoryId
    );
  }
  ...
  function renderCategories() {
    ...
    const allItem = createCategoryElement(
      { id: "all", name: "All Variables" },
      state.data.variables.length
    );
    ...
    const uncatCount = state.data.variables.filter(
    ...
    otherCategories.forEach((cat) => {
      const varCount = state.data.variables.filter(
        (v) => v.category === cat.id
      ).length;
  ...
  function createCategoryElement(cat, count) {
    ...
    const handle = isDraggable
      ? `<i class="icon handle" title="Drag to reorder">${ICONS.drag}</i>`
  ...
  function renderVariables() {
    ...
    if (visibleVars.length === 0) {
      ...
      if (state.searchQuery) titleText = "Search Results";
        ...
        titleText =
          state.data.categories.find((c) => c.id === state.activeCategoryId)
            ?.name || "Variables";
    ...
    const titleCategory =
      state.activeCategoryId === "all"
        ...
        : state.data.categories.find((c) => c.id === state.activeCategoryId)
            ?.name;
    dom.variablesTitle.textContent = `${
      state.searchQuery ? "Search Results" : titleCategory
    } (${visibleVars.length})`;
  ...
  function renderEmptyState(isSearch) {
    if (isSearch) {
      ...
      document.getElementById("empty-import-code-btn").onclick =
        openCodeImportModal;
      document.getElementById("empty-import-json-btn").onclick =
        openJsonImportModal;
    }
  }
  ...
  function renderSearchResults(variables, container) {
    ...
    const allCategories = [
      ...
      ...state.data.categories.filter((c) => c.id !== "uncategorized"),
    ];
    ...
    categoryOrder.forEach((catId) => {
      if (groupedByCat[catId] && groupedByCat[catId].length > 0) {
        ...
        groupedByCat[catId].forEach((v) =>
          gridEl.appendChild(createVariableCard(v))
        );
  ...
  function createVariableCard(v) {
    ...
    card.innerHTML = `
            ...
            <input type="checkbox" data-action="toggle-select" ${
              state.selectedVariableIds.has(v.id) ? "checked" : ""
            }>
            <div class="color-swatch" style="background-color: ${
              isColor ? v.value : "transparent"
            };"></div>
  ...
  function renderPropertiesPanel() {
    ...
    if (selectedCount === 1) {
      ...
      dom.propTabs.forEach(
        (t) =>
          (t.style.display = t.dataset.tab === "categorize" ? "flex" : "none")
      );
  ...
  function renderEditForm() {
    if (state.selectedVariableIds.size !== 1) {
      dom.propEditContent.innerHTML =
        '<p style="color: var(--text-secondary); text-align: center; padding-top: 2rem;">Select a single variable to edit.</p>';
      return;
    }
    const id = state.selectedVariableIds.values().next().value;
  ...
  function updateVariablePanelHeader() {
    ...
    const visibleVars = getVisibleVariables();
    dom.selectAllVarsCheckbox.checked =
      visibleVars.length > 0 &&
      visibleVars.every((v) => state.selectedVariableIds.has(v.id));
  ...
  function setupEventListeners() {
    ...
    dom.clearCategorySelectionBtn.addEventListener(
      "click",
      clearCategorySelection
    );
    dom.bulkDeleteCategoryBtn.addEventListener(
      "click",
    ...
    dom.propTabs.forEach((tab) =>
      tab.addEventListener("click", () => showPropTab(tab.dataset.tab))
    );
    ...
    dom.variableValueInput.addEventListener("input", (e) => {
      if (/^#([0-9a-f]{3}){1,2}$/i.test(e.target.value))
        dom.variableColorPicker.value = e.target.value;
    });
    dom.variableColorPicker.addEventListener(
      "input",
      (e) => (dom.variableValueInput.value = e.target.value)
    );
  ...
  function handleSearch(e) {
    ...
    dom.clearSearchBtn.style.display = state.searchQuery
      ? "inline-flex"
      : "none";
      ...
      switch (action) {
        case "edit-category":
          ...
          setTimeout(
            () =>
              dom.categoriesList
                .querySelector(`.category-name-input[data-id="${id}"]`)
                ?.focus(),
            0
          );
          break;
  ...
  function handleAddCategory() {
    ...
    renderCategories();
    setTimeout(() => {
      const input = dom.categoriesList.querySelector(
        `.category-name-input[data-id="${newId}"]`
      );
      if (input) {
        input.focus();
        input.select();
  ...
  function handleCategoryKeyDown(e) {
    if (e.target.classList.contains("category-name-input")) {
      if (e.key === "Enter") {
        e.preventDefault();
        saveCategoryEdit(e.target.dataset.id);
      } else if (e.key === "Escape") {
        cancelCategoryEdit(e.target.dataset.id);
      }
    }
  }
  ...
  function toggleVariableSelection(id, isShiftKey, isCtrlOrCmdKey) {
    ...
    if (
      isShiftKey &&
      state.lastSelectedVarId &&
      state.lastSelectedVarId !== id
    ) {
      const visibleVarIds = getVisibleVariables().map((v) => v.id);
  ...
  function toggleCategorySelection(id, isShiftKey, isCtrlOrCmdKey) {
    ...
    const selectableCats = state.data.categories.filter(
      (c) => c.id !== "all" && c.id !== "uncategorized"
    );
    const catIds = selectableCats.map((c) => c.id);

    if (
      isShiftKey &&
      state.lastSelectedCatId &&
      state.lastSelectedCatId !== id
    ) {
  ...
  function handleBulkCategoryDelete() {
    ...
    const details = `<ul>${Array.from(state.selectedCategoryIds)
      .map(
        (id) =>
          `<li>${
            state.data.categories.find((c) => c.id === id)?.name || "unknown"
          }</li>`
      )
      .join("")}</ul>`;
    ...
    openDeleteConfirmModal(
      ...
      () => {
        ...
        state.data.variables = state.data.variables.filter(
          (v) => !varIdsToDelete.has(v.id)
        );
        state.data.categories = state.data.categories.filter(
          (c) => !state.selectedCategoryIds.has(c.id)
        );
  ...
  function saveCategoryEdit(id) {
    const input = dom.categoriesList.querySelector(
      `.category-name-input[data-id="${id}"]`
    );
    if (!input) return;
  ...
  function handleDeleteCategory(id) {
    const category = state.data.categories.find((c) => c.id === id);
    if (!category) return;
    const varsInCatCount = state.data.variables.filter(
      (v) => v.category === id
    ).length;
  ...
  function handleBulkDelete() {
    ...
    const details = `<ul>${Array.from(state.selectedVariableIds)
      .map(
        (id) =>
          `<li>--${
            state.data.variables.find((v) => v.id === id)?.name || "unknown"
          }</li>`
      )
      .join("")}</ul>`;
    openDeleteConfirmModal(
      ...
      () => {
        ...
        state.data.variables = state.data.variables.filter(
          (v) => !state.selectedVariableIds.has(v.id)
        );
        state.selectedVariableIds.clear();
  ...
  function handleBulkMove() {
    ...
    state.data.variables.forEach((v) => {
      if (state.selectedVariableIds.has(v.id)) v.category = targetCatId;
    });
    const count = state.selectedVariableIds.size;
    const catName = state.data.categories.find(
      (c) => c.id === targetCatId
    ).name;
    clearSelection();
  ...
  function openVariableModal(id = null, isDuplicating = false) {
    ...
    if (id) {
      ...
      dom.variableCategorySelect.value =
        state.activeCategoryId !== "all"
  ...
  function openDeleteConfirmModal(
    ...
    dom.deleteConfirmBtn.parentNode.replaceChild(
      newConfirmBtn,
      dom.deleteConfirmBtn
    );
    ...
    dom.deleteConfirmCancelBtn.parentNode.replaceChild(
      newCancelBtn,
      dom.deleteConfirmCancelBtn
    );
  ...
  function openCodeImportModal() {
    ...
    document.getElementById("css-import-category-select").value =
      "uncategorized";

    // Setup import handler
    document.getElementById("css-import-btn").onclick =
      handleCssImportWithCategory;
  ...
  function showCssExportCategorySelection() {
    ...
    const categoriesWithVars = state.data.categories
      .map((cat) => ({
        ...cat,
        varCount: state.data.variables.filter((v) => v.category === cat.id)
          .length,
      }))
  ...
  function proceedWithCssExport(selectedCategoryIds) {
    ...
    document
      .getElementById("css-export-category-filter")
      .addEventListener("change", (e) => {
        const selectedValue = e.target.value;
        let newCssString;

        if (selectedValue === "all") {
          newCssString = generateCssForCategories(
            window.selectedExportCategories
          );
        } else {
  ...
  function generateCssForCategories(categoryIds) {
    ...
    categoryIds.forEach((catId) => {
      const category = state.data.categories.find((c) => c.id === catId);
      const varsInCategory = state.data.variables.filter(
        (v) => v.category === catId
      );
  ...
  function handleCssImport() {
    ...
    if (newVarsCount > 0)
      saveAndRender(`${newVarsCount} new variables imported.`);
  ...
  function handleCssImportWithCategory() {
    ...
    if (newVarsCount > 0) {
      const categoryName =
        state.data.categories.find((c) => c.id === selectedCategoryId)?.name ||
        "Unknown";
  ...
  function showImportModeModal(importedData, callback) {
    // Clean up any existing modal
    const existingModal = document.getElementById("import-mode-modal");
    if (existingModal) {
      document.body.removeChild(existingModal);
    }

    const importedVarCount = importedData.variables?.length || 0;
    const importedCatCount = (
      importedData.categories?.filter((c) => c.id !== "uncategorized") || []
    ).length;
    const existingVarCount = state.data.variables.length;
    const existingCatCount = state.data.categories.filter(
      (c) => c.id !== "uncategorized"
    ).length;
    ...
    const modalContent = `
            <div class="modal">
                ...
                <p style="margin-bottom: 1.5rem; color: var(--text-secondary);">
                    Choose how to import the selected file:
                </p>
                
                <div style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 2rem;">
                    <!-- Replace Mode Option -->
                    <label class="import-mode-option" style="
                        display: flex;
                    ...
                    ">
                        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                            <input type="radio" name="import-mode" value="replace" id="import-mode-replace" checked
                                   style="margin: 0;">
                            <strong style="font-size: 15px;">Replace All</strong>
                        </div>
                        <p style="margin: 0 0 0 2rem; font-size: 13px; color: var(--text-secondary); line-height: 1.5;">
                            Remove all existing data and replace with imported content.
                            <span style="color: var(--accent-danger); display: block; margin-top: 0.25rem;">
                                ⚠️ This will delete your ${existingVarCount} variables and ${existingCatCount} categories.
                            </span>
                        </p>
                    </label>
                    ...
                    ">
                        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                            <input type="radio" name="import-mode" value="merge" id="import-mode-merge"
                                   style="margin: 0;">
                            <strong style="font-size: 15px;">Merge & Keep Existing</strong>
                        </div>
                        <p style="margin: 0 0 0 2rem; font-size: 13px; color: var(--text-secondary); line-height: 1.5;">
                            Add ${importedVarCount} new variables and ${importedCatCount} categories to your existing collection.
                ...
                <div class="modal-actions">
                    <button type="button" class="main-btn btn-secondary" data-modal-close>Cancel</button>
                    <button type="button" class="main-btn btn-primary" id="confirm-import-mode-btn">Continue</button>
                </div>
            </div>
        `;
    ...
    const modalEl = document.createElement("div");
    modalEl.className = "modal-overlay";
    modalEl.id = "import-mode-modal";
    modalEl.innerHTML = modalContent;
    document.body.appendChild(modalEl);

    openModal("import-mode-modal");
    setupModalCloseHandlers("import-mode-modal");

    // Add hover effects to options
    const options = modalEl.querySelectorAll(".import-mode-option");
    options.forEach((option) => {
      ...
      option.addEventListener("click", () => {
        ...
        // Reset all options
        options.forEach((opt) => {
          opt.style.borderColor = "var(--border-secondary)";
    ...
    document.getElementById(
      "import-mode-replace"
    ).parentElement.style.borderColor = "var(--accent-primary)";
    document.getElementById(
      "import-mode-replace"
    ).parentElement.style.backgroundColor = "var(--bg-secondary)";

    // Handle confirmation
    document.getElementById("confirm-import-mode-btn").onclick = () => {
  ...
  function mergeImportedData(importedData) {
    const existingVarNames = new Set(state.data.variables.map((v) => v.name));
    const existingCatNames = new Map(
      state.data.categories.map((c) => [c.name.toLowerCase(), c])
    );
    ...
    const mergeResults = {
      ...
      addedCats: 0,
      renamedCats: 0,
      categoryIdMap: new Map(), // oldId -> newId for remapping variables
    };

    // Step 1: Process categories
    const importedCategories = importedData.categories.filter(
      (c) => c.id !== "uncategorized"
    );

    importedCategories.forEach((importedCat) => {
      const nameLower = importedCat.name.toLowerCase();
      const existingCat = existingCatNames.get(nameLower);

      if (existingCat) {
        // Category name exists - check if IDs match
        if (existingCat.id === importedCat.id) {
          // Same category, no action needed
          mergeResults.categoryIdMap.set(importedCat.id, existingCat.id);
        } else {
          // Different category with same name - rename and add
          const newCatId = generateId();
          const renamedCat = {
            id: newCatId,
            name: `${importedCat.name} (Imported)`,
          };
          state.data.categories.push(renamedCat);
          existingCatNames.set(renamedCat.name.toLowerCase(), renamedCat);
          mergeResults.categoryIdMap.set(importedCat.id, newCatId);
          mergeResults.addedCats++;
          mergeResults.renamedCats++;
        }
      } else {
        // New category - check if ID conflicts
        const idExists = state.data.categories.some(
          (c) => c.id === importedCat.id
        );
        if (idExists) {
          // ID collision (rare) - generate new ID
          const newCatId = generateId();
          const newCat = {
            id: newCatId,
          ...
          state.data.categories.push(newCat);
          existingCatNames.set(nameLower, newCat);
          mergeResults.categoryIdMap.set(importedCat.id, newCatId);
          mergeResults.addedCats++;
        } else {
          // Completely new category - add as-is
          state.data.categories.push({ ...importedCat });
          existingCatNames.set(nameLower, importedCat);
          mergeResults.categoryIdMap.set(importedCat.id, importedCat.id);
          mergeResults.addedCats++;
        }
      }
    });
    ...
    importedData.variables.forEach((importedVar) => {
      if (existingVarNames.has(importedVar.name)) {
        // Variable name exists - skip to prevent overwrite
        mergeResults.skippedVars++;
      } else {
        // New variable - remap category if needed
        const newCategoryId =
          mergeResults.categoryIdMap.get(importedVar.category) ||
          importedVar.category;

        // Validate category exists
        const categoryExists = state.data.categories.some(
          (c) => c.id === newCategoryId
        );
        const finalCategoryId = categoryExists
          ? newCategoryId
          : "uncategorized";
        ...
        state.data.variables.push(newVar);
        existingVarNames.add(importedVar.name);
        mergeResults.addedVars++;
      }
    });
  ...
  function showMergeSummaryModal(mergeResults) {
    // Clean up any existing modal
    const existingModal = document.getElementById("merge-summary-modal");
    if (existingModal) {
      document.body.removeChild(existingModal);
    }

    const hasConflicts =
      mergeResults.skippedVars > 0 || mergeResults.renamedCats > 0;
    ...
    const modalContent = `
            <div class="modal">
                ...
                <div style="margin-bottom: 1.5rem;">
                    <h4 style="color: var(--accent-secondary); margin-bottom: 0.75rem; font-size: 15px;">
                    ...
                    ">
                        <p style="margin: 0 0 0.5rem 0; color: var(--text-primary);">
                            <strong>${
                              mergeResults.addedVars
                            }</strong> new variables added
                        </p>
                        <p style="margin: 0; color: var(--text-primary);">
                            <strong>${
                              mergeResults.addedCats
                            }</strong> new categories added
                        </p>
                    </div>
                </div>
                ...
                ${
                  hasConflicts
                    ? `
                <div style="margin-bottom: 1.5rem;">
                    <h4 style="color: var(--accent-warning); margin-bottom: 0.75rem; font-size: 15px;">
                        ⚠ Conflicts Resolved
                    </h4>
                    <div style="
                        background: var(--bg-secondary);
                    ...
                    ">
                        ${
                          mergeResults.skippedVars > 0
                            ? `
                        <p style="margin: 0 0 0.5rem 0; color: var(--text-secondary); font-size: 13px;">
                            <strong style="color: var(--text-primary);">${mergeResults.skippedVars}</strong> duplicate variables skipped
                            <span style="display: block; margin-top: 0.25rem; font-size: 12px;">
                                Variables with matching names were not imported to preserve your existing data.
                            </span>
                        </p>
                        `
                            : ""
                        ...
                        ${
                          mergeResults.renamedCats > 0
                            ? `
                        <p style="margin: 0; color: var(--text-secondary); font-size: 13px;">
                            <strong style="color: var(--text-primary);">${mergeResults.renamedCats}</strong> categories renamed
                            <span style="display: block; margin-top: 0.25rem; font-size: 12px;">
                                Categories with matching names were renamed to "Name (Imported)".
                            </span>
                        </p>
                        `
                            : ""
                ...
                <div class="modal-actions">
                    <button type="button" class="main-btn btn-primary" data-modal-close style="width: 100%;">
                        Close
                    </button>
                </div>
            </div>
    ...
    openModal("merge-summary-modal");
    setupModalCloseHandlers("merge-summary-modal");
    ...
    closeButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        setTimeout(() => {
          const modal = document.getElementById("merge-summary-modal");
          if (modal) document.body.removeChild(modal);
        }, 300);
      });
    });
  ...
  function executeReplace(importedData) {
    const finalCategories = [
      ...state.data.categories.filter((c) => c.id === "uncategorized"),
    ];
    const importedUserCategories = importedData.categories.filter(
      (c) => c.id !== "uncategorized"
    );
    ...
    saveAndRender("Variables imported successfully.");
  ...
  function handleJsonFile(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        const hasExistingData = state.data.variables && Array.isArray(state.data.variables) && state.data.variables.length > 0;
        
        if (!hasExistingData) {
          // Direct import without modal for empty state
          executeReplace(importedData);
        } else {
          // Existing flow: show mode selection modal
          showImportModeModal(importedData, (selectedMode) => {
            if (selectedMode === "replace") {
              executeReplace(importedData);
            } else if (selectedMode === "merge") {
              // New merge logic
              const mergeResults = mergeImportedData(importedData);
              ...
              setTimeout(() => {
                showMergeSummaryModal(mergeResults);
              }, 400);
            }
          });
        }
        ...
        showToast(
          "Invalid JSON format. Please use a compatible file.",
          "error"
        );
      }
    };
  ...
  function exportAsJson() {
    const uncategorizedVars = state.data.variables.filter(
      (v) => v.category === "uncategorized"
    );
  ...
  function showUncategorizedRenameModal(callback) {
    ...
    document.getElementById("confirm-rename-btn").onclick = () => {
      const newName = document
        .getElementById("uncategorized-rename-input")
        .value.trim();
      if (!newName) {
      ...
      const modalToRemove = document.getElementById(
        "uncategorized-rename-modal"
      );
      if (modalToRemove) {
    ...
    const categoriesWithVars = state.data.categories
      .map((cat) => ({
        ...
        varCount: state.data.variables.filter((v) => v.category === cat.id)
          .length,
        displayName:
          cat.id === "uncategorized" && window.uncategorizedRename
            ? window.uncategorizedRename
            : cat.name,
      }))
      .filter((cat) => cat.varCount > 0);
  ...
  function proceedWithJsonExport(selectedCategoryIds) {
    ...
    selectedCategoryIds.forEach((catId) => {
      const category = state.data.categories.find((c) => c.id === catId);
      const categoryVars = state.data.variables.filter(
        (v) => v.category === catId
      );

      if (category && categoryVars.length > 0) {
        // When uncategorized is renamed, create a new category with a unique ID
        const isUncategorizedRenamed =
          catId === "uncategorized" && window.uncategorizedRename;
  ...
  function setupCategorySortable() {
    ...
    categorySortable = Sortable.create(dom.categoriesList, {
      ...
      animation: 150,
      filter:
        '.category-item[data-id="all"], .category-item[data-id="uncategorized"]',
      onEnd: (evt) => {
        const { oldIndex, newIndex } = evt;
        // Adjust for the 2 static items at the top
        const movableCats = state.data.categories.filter(
          (c) => c.id !== "uncategorized"
        );
  ...
  function setupVariableSortable() {
    ...
    variableSortable = Sortable.create(container, {
      ...
      onEnd: (evt) => {
        ...
        const globalFromIndex = state.data.variables.findIndex(
          (v) => v.id === movedVarId
        );
